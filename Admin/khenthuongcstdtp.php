...
// Lấy tổng số bản ghi
$totalRecords = DanhgiaCN::layTongSoDanhGiaTP($conn);

// Tính tổng số trang
$totalPages = ceil($totalRecords / $recordsPerPage);

// Lấy dữ liệu cho trang hiện tại
$data = DanhgiaCN::layDanhGiaTPPhanTrang($conn, $startFrom, $recordsPerPage);
...
<nav aria-label="Page navigation">
    <ul class="pagination">
        <?php if ($currentPage > 1): ?>
            <li class="page-item">
                <a class="page-link" href="?p=khenthuongcstdtp&page=<?php echo $currentPage - 1; ?>" aria-label="Previous">
        ...
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <li class="page-item <?php echo $i == $currentPage ? 'active' : ''; ?>">
                <a class="page-link" href="?p=khenthuongcstdtp&page=<?php echo $i; ?>"><?php echo $i; ?></a>
            </li>
        <?php endfor; ?>

        <?php if ($currentPage < $totalPages): ?>
            <li class="page-item">
                <a class="page-link" href="?p=khenthuongcstdtp&page=<?php echo $currentPage + 1; ?>" aria-label="Next">
...
<script>
    ...
    console.log('Raw response from server:', textResponse);
        ...
        data.forEach(item => {
            ...
            const validDanhGia = ['Chiến Sĩ Thi Đua Thành Phố'];
