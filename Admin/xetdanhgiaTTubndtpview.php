...
// Lấy tổng số bản ghi
$totalRecords = DanhgiaTT::layTongSoDanhGiaubndtp($conn);

// Tính tổng số trang
$totalPages = ceil($totalRecords / $recordsPerPage);

// Lấy dữ liệu cho trang hiện tại
$data = DanhgiaTT::layDanhGiaubndtpPhanTrang($conn, $startFrom, $recordsPerPage);
...
<?php if(isset($_SESSION['TenTk']) && $_SESSION['VaiTro'] === 'Quản Trị'){?> 
    <a class="btn btn-primary btn-lg" href="<?php echo " $baseUrl?p=danhgiaTThubndtpthem"; ?>">Xét Đánh Giá Tập Thể</a>
<?php
}
?>
...
<nav aria-label="Page navigation">
    <ul class="pagination">
        <?php if ($currentPage > 1): ?>
            <li class="page-item">
                <a class="page-link" href="?p=danhgiaTThubndtp&page=<?php echo $currentPage - 1; ?>" aria-label="Previous">
        ...
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <li class="page-item <?php echo $i == $currentPage ? 'active' : ''; ?>">
                <a class="page-link" href="?p=danhgiaTThubndtp&page=<?php echo $i; ?>"><?php echo $i; ?></a>
            </li>
        <?php endfor; ?>

        <?php if ($currentPage < $totalPages): ?>
            <li class="page-item">
                <a class="page-link" href="?p=danhgiaTThubndtp&page=<?php echo $currentPage + 1; ?>" aria-label="Next">
